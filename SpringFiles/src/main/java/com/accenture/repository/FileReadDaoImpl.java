...
@Repository
@Transactional
public class FileReadDaoImpl implements FileReadDao {
	...
	private boolean readDataFromCsv(MultipartFile file) {
		try {
			...
			for (String[] row : allData) {
				entityManager.persist(new User(row[0], row[1], row[2], row[3],
	...
	public boolean createPdf(List<User> listuser, ServletContext context, HttpServletRequest request,
		...
		try {
			...
			for (User user : listuser) {
				...
				PdfPCell firstNameValue = new PdfPCell(new Paragraph(user.getFirstName(), tableBody));
				...
				PdfPCell lastNamevalue = new PdfPCell(new Paragraph(user.getLastName(), tableBody));
				...
				PdfPCell phoneNumbervalue = new PdfPCell(new Paragraph(user.getPhoneNumber(), tableBody));
	...
	public boolean createExcel(List<User> listuser, ServletContext context, HttpServletRequest request,
		...
		try {
			...
			FileOutputStream OutputStream = new FileOutputStream(file + "/" + "user" + ".xls");
			...
			headercellStyle.setFillForegroundColor(HSSFColor.HSSFColorPredefined.GREY_50_PERCENT.getIndex());
			headercellStyle.setFillPattern(FillPatternType.SOLID_FOREGROUND);

			HSSFRow headrow = worksheet.createRow(0);
			// Header decoration
			HSSFCell Firstname = headrow.createCell(0);
			...
			HSSFCell email = headrow.createCell(2);
	...
	public boolean createJson(List<User> listuser, ServletContext context) {
		...
		ObjectMapper mapper = new ObjectMapper();
		try {
			mapper.writeValue(file, listuser);
	...
	public boolean createCSV(List<User> listuser, ServletContext context) {
		...
		try {
			...
			for (User user : listuser) {
				data.add(new String[] { user.getFirstName(), user.getLastName(), user.getEmail(),
						user.getPhoneNumber() });
			}
